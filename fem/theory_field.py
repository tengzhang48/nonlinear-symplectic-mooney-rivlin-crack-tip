...
Leading deformed map, with amplitude P:
    ...
    y1(r,theta) = P^(-1/2) * r^(5/4) * g(theta)                     (in-plane)
...
leading profile.  The amplitude scaling y2 ~ P, y1 ~ P^(-1/2) is the constrained
solution's one-parameter family (the Q1 ~ P^(-1/2) anomaly).
...
def deformed(r, theta, P):
    """Leading deformed coordinates (y1, y2) at amplitude P."""
    r = np.asarray(r, float)
    y2 = P * r ** A2 * f_open(theta)
    y1 = P ** -0.5 * r ** A1 * g_inplane(theta)
    return y1, y2


def u_theory_factory(P):
