...
`python figures/make_figures.py` rebuilds the nine main figure pairs in
...
| `fig_hierarchy.{png,pdf}` | `figures/make_figures.py` | Live functions from `analysis/symplectic_dae.py` and `analysis/leading_field.py` |
...
| `fig_solution_compare.{png,pdf}` | `figures/make_figures.py` | `data/fem/disk/fem_case_MR_lam20.json`; `data/analytic/mr_leading_profile.npz` |
| `fig_tip_shape.{png,pdf}` | `figures/make_figures.py` | `fem_case_MR_lam20.json`, `fem_case_NH_lam20.json` |
...
mesh, and material-ratio sweeps. The main figure preflight enumerates only the
subset it reads.
